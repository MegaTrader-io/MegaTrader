/* modal.js — unified error modal + targeted blocks */
window.MEGATRADER = window.MEGATRADER || {};

/* =================== Config =================== */
// Modales que NO deben mostrarse (por ahora)
const MT_BLOCK_ALWAYS = new Set(['mt-breach-alert-modal', 'mt-agreement-modal']);
...
window.MEGATRADER.showModal = function (modalId) {
  ...
  // Nunca abrir los bloqueados
  if (MT_BLOCK_ALWAYS.has(modalId)) return;
...
function autoshow(modal){
  ...
  // Saltar autoshow para los bloqueados
  if (MT_BLOCK_ALWAYS.has(modalId)) return;

  const tryShow = () => window.MEGATRADER.showModal(modalId);
...
/* Bloqueo por evento Bootstrap: impedir que se abran los bloqueados */
document.body.addEventListener('show.bs.modal', function(ev){
  const id = ev.target?.id || '';
  if (MT_BLOCK_ALWAYS.has(id)) {
    ev.preventDefault();
    hideModalEl(ev.target);
  }
}, true);

/* Enforcer DOM: si por cualquier razón quedan "show", cerrarlos si son bloqueados */
(function setupModalEnforcer(){
  const obs = new MutationObserver(muts => {
    for (const m of muts) {
      if (m.type === 'attributes' && m.target?.classList?.contains('modal')) {
        const id = m.target.id || '';
        if (MT_BLOCK_ALWAYS.has(id) && m.target.classList.contains('show')) {
          hideModalEl(m.target);
        }
      }
      m.addedNodes && m.addedNodes.forEach(n => {
        if (n.nodeType !== 1) return;
        if (n.matches?.('.modal.show') && MT_BLOCK_ALWAYS.has(n.id || '')) hideModalEl(n);
        n.querySelectorAll?.('.modal.show').forEach(el => {
          if (MT_BLOCK_ALWAYS.has(el.id || '')) hideModalEl(el);
        });
      });
    }
  });
...
(function ($, window, document) {
  ...
  MT.showErrorModal = function (title, message, opts) {
    ...
    // Asegura que los bloqueados no queden abiertos detrás
    closeModalsByIds(MT_BLOCK_ALWAYS);
