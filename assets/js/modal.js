...
function handleModalWithNotice() {
  ...
  document.body.addEventListener("hidden.bs.modal", function (event) {
    document
      .querySelectorAll(".woocommerce-notices-wrapper-modal-open")
      .forEach((wrapper) => {
...
(function ($, window, document) {
  ...
  MT.showErrorModal = function (title, message, opts) {
    ...
    setText(
      headEl,
      opts.headline || (title && title.length <= 12 ? title : "Ups!")
    );
    ...
    try {
      if (window.bootstrap && typeof bootstrap.Modal === "function") {
        (
          window.__mtErrModal ||
          (window.__mtErrModal = new bootstrap.Modal(el, {
            backdrop: "static",
          }))
        ).show();
        ...
        // Cierre manual
        $(el)
          .find('[data-bs-dismiss="modal"], .mt-modal__close')
          .one("click", function () {
            MT.hideErrorModal();
          });
      }
  ...
  // API global para usar en todos los JS
  window.MEGATRADER = window.MEGATRADER || {};
  ...
  MT.hideErrorModal = function () {
    ...
    try {
      if (window.bootstrap && typeof bootstrap.Modal === "function") {
        (
          window.__mtErrModal || (window.__mtErrModal = new bootstrap.Modal(el))
        ).hide();
      } else {
