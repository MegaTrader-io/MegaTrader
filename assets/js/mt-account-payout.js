(function () {
  ...
  // =========================
  // Prefetch / caché con TTL
  // =========================
  var CACHE_TTL_MS = 15000; // 15s
  var MT_PAYOUT_CACHE = null; // { ts, data }
  ...
  function init() {
    ...
    // hidratar al abrir (y refrescar cada vez)
  ...
  function renderDropdown(mount, data) {
    ...
    btn.innerHTML =
      ...
      escapeHtml(selItem.accountName || "—") +
      "</span>" +
      "</span>" +
      '<span class="d-flex align-items-center gap-3 right-group">' +
      '  <span class="' +
      escapeHtml(selItem.badge.class) +
      '">' +
      escapeHtml(selItem.badge.text) +
      "</span>" +
  ...
  // =========================
  // Lógica de límites + validación de input
  // =========================
  function applyLimitsAndValidation(item, maxEl, inputEl, errEl, contBtn) {
    ...
    // Mostrar máximo (texto + data attr)
    if (maxEl) {
    ...
    // Estado del input y botón
    if (inputEl) {
      inputEl.value = ""; // limpiar valor al cambiar cuenta
    ...
    // No hay nada que validar si no es elegible o no hay max
    ...
    function validate() {
      ...
      if (!raw) {
        setError(""); // vacío: sin error visual, solo botón off
        return;
      }
      ...
      if (m <= 0) {
        setError("This account is not eligible for payout at the moment.");
        return;
      }
      if (val > m) {
        setError("Amount exceeds the maximum allowed for this payout.");
        return;
      }
    ...
    if (contBtn) {
      contBtn.addEventListener(
        ...
        function () {
          ...
          // TODO: continuar flujo (submit/AJAX) usando:
          //  - accountId => #mt-payout-acc-btn[data-account-id]
          //  - amount    => #mt-payout-amount.value
        },
        { passive: true }
      );
    }
