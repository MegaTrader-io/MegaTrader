(function () {
  ...
  function showGlobalError(headline, message, opts) {
    try {
      if (window.MEGATRADER && typeof window.MEGATRADER.showError === "function") {
        ...
        setTimeout(function () {
          var lastBackdrop = document.querySelector('.modal-backdrop:last-of-type');
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () { prefetchPayoutData(false); wirePrefetchTriggers(); }, { once: true });
  } else { prefetchPayoutData(false); wirePrefetchTriggers(); }
  ...
  function switchToStep(step) {
    ...
    if (step === 2) {
      ...
      backBtn.classList.remove("d-none"); cancelBtn.classList.add("d-none");
      contBtn.classList.add("d-none"); confirmBtn.classList.remove("d-none");
      confirmBtn.disabled = false;
    } else {
      s2.classList.add("d-none"); s1.classList.remove("d-none");
      cancelBtn.classList.remove("d-none"); backBtn.classList.add("d-none");
      contBtn.classList.remove("d-none"); confirmBtn.classList.add("d-none");
    }
  }
  ...
  function buildItemRow(item) {
    ...
    btn.innerHTML =
      ...
      '  <span class="badge-mega badge-mega-default badge-mega-sm">'+ escapeHtml(item.accountName || "—") +'</span>' +
      '</span>' +
  ...
  function hydrateOnce(force) {
    ...
    if (MT_PAYOUT_CACHE && Array.isArray(MT_PAYOUT_CACHE.data && MT_PAYOUT_CACHE.data.items)) {
  ...
  function renderDropdown(mount, data) {
    if (!data.items.length) { mount.innerHTML = '<div class="text-muted">No funded active accounts found.</div>'; return; }
    ...
    var wrap = document.createElement("div"); wrap.className = "dropdown w-100";
    ...
    btn.innerHTML =
      ...
      '  <span id="mt-payout-acc-name-badge" class="badge-mega badge-mega-default badge-mega-sm">'+ (selItem.accountName || "—") +'</span>' +
      '</span>' +
    ...
    select.id = "mt-payout-acc-select"; select.className = "d-none";
    data.items.forEach(function (it) {
      var opt = document.createElement("option");
      opt.value = it.id; opt.textContent = it.accountName || "—";
      if (it.id === selItem.id) opt.selected = true;
      select.appendChild(opt);
    });

    mount.innerHTML = "";
    wrap.appendChild(btn); wrap.appendChild(ul); wrap.appendChild(select);
    ...
    ul.addEventListener("click", function (e) {
      var opt = e.target.closest(".mt-payout-option"); if (!opt) return;
      var id = opt.getAttribute("data-value");
      var item = data.items.find(function (it) { return it.id === id; }); if (!item) return;
      ...
      if (nameBadge) {
        ...
        nameBadge.className = "badge-mega badge-mega-default badge-mega-sm";
      }
  ...
  function applyLimitsAndValidation(item, maxEl, inputEl, errEl, contBtn) {
    ...
    if (inputEl) {
      inputEl.value = ""; inputEl.classList.remove("is-invalid");
      inputEl.disabled = !eligible || !(maxUI > 0);
      if (maxUI > 0) inputEl.setAttribute("max", String(Math.floor(maxUI))); else inputEl.removeAttribute("max");
    }
    ...
    var max = function () { var v = parseFloat((maxEl && maxEl.dataset && maxEl.dataset.max) || "0"); return Number.isFinite(v)?v:0; };
    ...
    function validate() {
      ...
      if (val < minUI) {
        setError(((I18N && I18N.withdrawalAmountBelowMin) || "Amount is below the minimum withdrawal for this account.") + (minUI>0?(" ("+fmtMoney(minUI)+" min)"):""));
        return;
      }
      setError("");
    ...
    if (contBtn && !contBtn._wired) {
      ...
      contBtn.addEventListener("click", function () {
        ...
        validate(); if (contBtn.disabled) return;
...
if (confirmBtn && !confirmBtn._wired) {
    ...
    fetch(url, {
      ...
      .then(function (r) { 
        ...
        return r.text().then(function (t) {
          var json = null; try { json = JSON.parse(t); } catch(e){}
          return { status: status, raw: t, json: json };
        });
      })
      .then(function (resp) {
        console.log("[PAYOUT] ←", resp);
        var status = resp.status, res = resp.json;

        if (status === 409) {
          showGlobalError("Payout unavailable",
            (res && (res.data?.message || res.message)) || "You already have a payout request in progress.",
            { code: "409" });
          confirmBtn.disabled = false;
          return;
        }
  ...
  function init() {
    var modal = document.getElementById("mt-request-payout-modal");
    if (!modal) return;
    modal.addEventListener("show.bs.modal", function () { switchToStep(1); hydrateOnce(true); });
    var backBtn = document.getElementById("mt-payout-back");
    if (backBtn) backBtn.addEventListener("click", function () { switchToStep(1); }, { passive: true });
  }
