...
(function () {
  ...
  var I18N = (typeof window !== "undefined" && window.MT_PAYOUT_I18N) || {};
  var CACHE_TTL_MS = 15000; // TTL del caché (ms)
  var MT_PAYOUT_CACHE = null;
  var _mtp_inflight = null; // evita requests simultáneos
  var _mtp_lastErrorTs = 0;
  ...
  function showPreloader(){try{if(window.jQuery&&window.jQuery(".preloader").length)window.jQuery(".preloader").fadeIn();}catch(e){}}
  function hidePreloader(){try{if(window.jQuery&&window.jQuery(".preloader").length)window.jQuery(".preloader").fadeOut();}catch(e){}}

  // ---------- Modal error ----------
  function showGlobalError(headline, message, opts) {
    try {
      if (window.MEGATRADER && typeof window.MEGATRADER.showError === "function") {
        window.MEGATRADER.showError(headline || "Request failed", message || "Unexpected error.", opts || {});
        setTimeout(function(){var b=document.querySelector(".modal-backdrop:last-of-type"); if(b)b.classList.add("mt-error");},0);
        return;
      }
      ...
      if (m) {
        ...
        var inst = window.bootstrap && window.bootstrap.Modal ? window.bootstrap.Modal.getOrCreateInstance(m, {backdrop:true}) : null;
        if (inst) {
          inst.show();
          setTimeout(function(){var b=document.querySelector(".modal-backdrop:last-of-type"); if(b)b.classList.add("mt-error");},0);
        } else {
  ...
  function looksLikeHtml(t){ return /<\/?[a-z][\s\S]*>/i.test(t); }
  ...
  function prefetchPayoutData(force) {
    ...
    if (_mtp_inflight) return _mtp_inflight;
    ...
    _mtp_inflight = fetch(AJAX_URL, { method: "POST", body: fd, credentials: "same-origin" })
      .then(function (r) { return r.text().then(function (t) { return {status:r.status, raw:t}; }); })
      .then(function (resp) {
        var status = resp.status, raw = resp.raw;
        // WordPress devuelve "0" en errores o nonce inválido
        if (raw === "0" || status < 200 || status >= 300) {
          throw new Error("admin-ajax error: status="+status+" body="+raw.slice(0,120));
        }
        if (looksLikeHtml(raw)) {
          throw new Error("Received HTML (maybe login or error page)");
        }
        ...
        // Acepta estructura alternativa {items:[]}
        if (res && Array.isArray(res.items)) {
          var data = { items: res.items, selected: res.selected || null };
          MT_PAYOUT_CACHE = { ts: Date.now(), data: data };
          return data;
        }
        throw new Error("Unexpected payload shape");
      })
      .catch(function (err) {
        console.warn("[PAYOUT] prefetch failed:", err && err.message ? err.message : err);
        // Reintento único con pequeño backoff si no hay caché
        if (!MT_PAYOUT_CACHE) {
          return new Promise(function (resolve) {
            setTimeout(function () {
              _mtp_inflight = null;
              prefetchPayoutData(true).then(resolve).catch(function(){
                resolve(null);
              });
            }, 350);
          });
      ...
      .finally(function(){ _mtp_inflight = null; });

    return _mtp_inflight;
  }
  ...
  function wirePrefetchTriggers() {
    ...
    triggers.forEach(function (btn) {
      ...
      // También cuando el botón gana foco (accesibilidad)
      btn.addEventListener("focus", function () { prefetchPayoutData(false); }, { passive: true });
    });
  }

  // Solo intentamos prefetch si el modal existe; no mostramos error aún
  function lazyWarmup() {
    var modal = document.getElementById("mt-request-payout-modal");
    if (!modal) return;
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", lazyWarmup, { once: true });
  } else {
    lazyWarmup();
  }
  ...
  function ensureConfirmButton() {
    var footer = document.querySelector("#mt-request-payout-modal .modal-footer");
    ...
    if (!confirmBtn) {
      ...
      confirmBtn.className = "flex-1-1-0 mt-btn mt-btn--md mt-btn--primary d-none";
      confirmBtn.textContent = (I18N && I18N.confirmButton) || "Confirm Request";
  ...
  function buildItemRow(item) {
    ...
    btn.innerHTML =
      ...
      '  <span class="fw-bold text-white text-base text-uppercase">'+ escapeHtml(item.accountSize || "—") +'</span>' +
      '  <span class="mt-badge mt-badge-light mt-badge-rounded-sm">'+ escapeHtml(item.accountName || "—") +'</span>' +
      '</span>' +
      '<span class="d-flex align-items-center gap-3 right-group d-none">' +
      '  <span class="'+ escapeHtml(item.badge && item.badge.class || "badge-mega-default") +'">'+ escapeHtml(item.badge && item.badge.text || "") +'</span>' +
      '</span>';
    li.appendChild(btn);
    return li;
  }
  ...
  function hydrateOnce(force) {
    ...
    // Placeholder neutro mientras llega el fetch
    ...
    prefetchPayoutData(!!force).then(function (data) {
      if (data && Array.isArray(data.items)) {
        ...
        // Solo mostramos error si de verdad no hay caché ni items
        if (!data || !Array.isArray(data.items)) {
          mount.innerHTML = '<div class="text-danger">Error loading accounts</div>';
          _mtp_lastErrorTs = Date.now();
        }
      }
    });
  }
  ...
  function renderDropdown(mount, data) {
    ...
    btn.innerHTML =
      ...
      '  <span id="mt-payout-acc-size" class="fw-bold text-white text-base text-uppercase">'+ (selItem.accountSize || "—") +'</span>' +
      '  <span id="mt-payout-acc-name-badge" class="mt-badge mt-badge-light mt-badge-rounded-sm">'+ (selItem.accountName || "—") +'</span>' +
      '</span>' +
      '<span class="d-flex align-items-center gap-3 right-group d-none">' +
      '  <span class="'+ (selItem.badge && selItem.badge.class || "badge-mega-default") +'">'+ (selItem.badge && selItem.badge.text || "") +'</span>' +
      '</span>';

    var ul = document.createElement("ul");
    ul.id = "mt-payout-acc-menu";
    ul.className = "dropdown-menu w-100 p-0 rounded-12 mt-1 bg-1e1e1e border-gray mt-acc-menu";
    ...
    var modal = document.getElementById("mt-request-payout-modal");
    if (modal) modal.setAttribute("data-account-id", selItem.id || modal.getAttribute("data-account-id") || "");
    ...
    ul.addEventListener("click", function (e) {
      ...
      if (img && item.logo) img.src = item.logo;
        ...
        if (sizeNode) {
          ...
          newImg.src = item.logo; newImg.alt = "";
          newImg.style.width = "30px"; newImg.style.height = "30px"; newImg.style.borderRadius = "50%";
          holder.insertBefore(newImg, sizeNode);
        }
      }

      var elig = btn.querySelector(".right-group .badge-mega");
      if (elig) { elig.className = item.badge.class; elig.textContent = item.badge.text; }
    ...
    var backBtn = document.getElementById("mt-payout-back");
    if (backBtn) backBtn.addEventListener("click", function () { switchToStep(1); }, { passive: true });
  ...
  function wireMethodSelectors() {
    ...
    var btns = $all('#mt-payout-step1 .mt-btn[data-role="mt-method-btn"][data-value]', modal);

    btns.forEach(function (b) {
      b.addEventListener("click", function () {
        btns.forEach(function (x) { x.classList.remove("mt-btn--primary"); x.classList.add("mt-btn--secondary"); });
        b.classList.remove("mt-btn--secondary"); b.classList.add("mt-btn--primary");
        ...
        if (hf) hf.setAttribute("name", "email");
        var mHidden = document.getElementById("mt-payout-method");
        if (mHidden) mHidden.value = val;
      }, { passive: true });
    });

    var def = btns.find(function (x) { return (x.getAttribute("data-value") || "") === "Rise"; }) || btns[0];
    if (def) def.click();
  }
  ...
  function applyLimitsAndValidation(item, maxEl, inputEl, errEl, contBtn) {
    ...
    function validate() {
      ...
      if (!Number.isFinite(val) || val <= 0) { setError((I18N&&I18N.withdrawalAmountMinorZero)||"Enter a valid amount greater than 0."); return; }
      if (m <= 0) { setError((I18N&&I18N.withdrawalAmountNotEligible)||"This account is not eligible for payout at the moment."); return; }
      if (val > m) { setError((I18N&&I18N.withdrawalAmountError)||"Amount exceeds the maximum allowed for this payout."); return; }
      if (val < minUI) { setError(((I18N&&I18N.withdrawalAmountBelowMin)||"Amount is below the minimum withdrawal for this account.") + (minUI>0?" ("+fmtMoney(minUI)+" min)":"")); return; }
      setError("");
    ...
    if (contBtn && !contBtn._wired) {
      ...
      contBtn.addEventListener("click", function () {
        ...
        var modal = document.getElementById("mt-request-payout-modal");
        var email = (modal && modal.getAttribute("data-user-email")) || ($("#mt-payout-email") && $("#mt-payout-email").value) || "";
        ...
        var internalId = (accBtn && accBtn.getAttribute("data-account-id")) || (modal && modal.getAttribute("data-account-id")) || item.id;
        ...
        setTxt("mt-review-account", (accBtn && accBtn.getAttribute("data-platform-account-id")) || item.platformAccountId || "—");
        ...
        window.MT_PAYOUT_SUBMIT = {
          ...
          platformAccountId: (accBtn && accBtn.getAttribute("data-platform-account-id")) || item.platformAccountId || "",
          amount: amount, fee: fee, receive: receive, method: method, email: email
        };
    ...
    if (confirmBtn && !confirmBtn._wired) {
      ...
      confirmBtn.addEventListener("click", function () {
        ...
        var method  = st.method || ""; // “Rise”, etc.
        var email   = st.email  || "";

        if (!account || !amount || !method || !email) {
          var err = document.getElementById("mt-payout-error");
          if (err) { err.style.display = ""; err.textContent = "Account, amount, method and email are required."; }
          return;
        }

        var url = (function () {
          var base = (window.MT_PAYOUT_VARS && window.MT_PAYOUT_VARS.ajaxurl) || window.ajaxurl || "/wp-admin/admin-ajax.php";
          var q = new URLSearchParams();
          q.set("action", "mt_payouts_create");
          if (window.MT_PAYOUT_VARS && MT_PAYOUT_VARS.nonce) q.set("_wpnonce", MT_PAYOUT_VARS.nonce);
        ...
        .then(function (resp) {
          ...
          var parsed = safeJsonParse(raw);
          var res = parsed.ok ? parsed.json : null;

          if (status === 409) {
            showGlobalError("Payout unavailable", (res && (res.data?.message || res.message)) || "You already have a payout request in progress.", { code:"409" });
            confirmBtn.disabled = false;
            return;
          }

          if (status < 200 || status >= 300 || !res) {
            var msg = (res && (res.data?.message || res.data?.error)) || (res && res.message) || "Request failed.";
          ...
          if (res && res.success) {
            ...
            var msg2 = (res && (res.data?.message || res.data?.error)) || "Unknown error.";
        ...
        .catch(function (e) {
          ...
          showGlobalError("Network error", "Network/server error, please try again later.", {});
          confirmBtn.disabled = false;
        })
  ...
  function renderCongratsStep() {
    ...
    var msg = (I18N && I18N.congratsMessage) || "Your request has been successfully submitted";
    var subtitle = (I18N && I18N.congratsSubtitle) || "You’ll be notified once your request is approved.";
  ...
  // === Reset modal al cerrar y al abrir ===
  (function () {
    ...
    function snapshotModal() {
      ...
      _mtp_initSnapshot = { body: body?body.innerHTML:"", footer: footer?footer.innerHTML:"", title: title?title.textContent:"" };
    }
    ...
    function restoreModal() {
      ...
      if (amt) { amt.value = ""; amt.classList.remove("is-invalid"); }

      var confirmBtn = document.getElementById("mt-payout-confirm");
      if (confirmBtn) { confirmBtn.classList.add("d-none"); confirmBtn.disabled = false; }

      try { switchToStep(1); } catch(e){}
    }

    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", snapshotModal, { once: true });
    } else {
      snapshotModal();
    }
    ...
    document.addEventListener("hidden.bs.modal", function (ev) {
      if (ev.target && ev.target.id === "mt-request-payout-modal") {
        restoreModal();
        try { MT_PAYOUT_CACHE = null; } catch(e){}
      }
    });
  })();
