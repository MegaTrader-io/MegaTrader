...
function enqueue_billing_validation_script() {
    if (is_checkout()) {
        wp_enqueue_script(
            'billing-validation',
            get_stylesheet_directory_uri() . '/assets/js/billing-validation.js',
            array(),
            time(),
            true
        );
...
add_action('wp_enqueue_scripts', 'enqueue_billing_validation_script');
...
add_action( 'template_redirect', 'mt_redirect_my_account_dashboard' );
function mt_redirect_my_account_dashboard() {
    // 1) Solo usuarios logueados
    if ( ! is_user_logged_in() ) {
        return;
    }

    // 2) Solo en la página raíz de My Account
    if ( ! is_account_page() || WC()->query->get_current_endpoint() !== '' ) {
        return;
    }

    $user_id = get_current_user_id();

    // 3) Obtener todas las órdenes processing/completed
    $orders = wc_get_orders( array(
        'customer_id' => $user_id,
        'status'      => array( 'wc-processing', 'wc-completed' ),
        'limit'       => -1,
    ...
    // 4) Filtrar para quitar órdenes sólo de “reset-fee” o “activation-fee”
    $valid_orders = array_filter( $orders, function( $order ) {
        foreach ( $order->get_items() as $item ) {
            $product = $item->get_product();
            if ( ! $product ) {
    ...
    // 5) Si no hay órdenes válidas, dejamos que cargue el dashboard normal
    if ( empty( $valid_orders ) ) {
        return;
    }

    // 6) Tomar la última orden válida
    /** @var WC_Order $last_order */
    $last_order = array_values( $valid_orders )[0];

    // 7) ¿Tiene suscripción relacionada?
    if ( function_exists( 'wcs_get_subscriptions_for_order' ) ) {
        $subs = wcs_get_subscriptions_for_order( $last_order, array( 'order_type' => 'any' ) );
    } else {
        $subs = array();
    ...
    if ( ! empty( $subs ) ) {
        $sub          = reset( $subs );
        $redirect_url = wc_get_endpoint_url( 'view-subscription', $sub->get_id(), wc_get_page_permalink( 'myaccount' ) );
    } else {
        $redirect_url = wc_get_endpoint_url( 'view-order', $last_order->get_id(), wc_get_page_permalink( 'myaccount' ) );
    }
