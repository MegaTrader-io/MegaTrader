...
add_action('rest_api_init', function () {
  ...
  register_rest_route('custom/v1', '/best-coupon-in-batch', array(
    'methods' => 'GET',
    'callback' => 'mt_get_products_with_categories_in_batch',
    'permission_callback' => '__return_true'
  ));
});
...
if (!function_exists('mt_get_products_with_categories_in_batch')) {
  function mt_get_products_with_categories_in_batch(WP_REST_Request $request)
  {
    ...
    foreach ($product_ids as $product_id) {
      $id = (int)$product_id;
      $result = mt_get_best_coupon_for_variation($id);
