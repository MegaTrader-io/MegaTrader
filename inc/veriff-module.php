...
function mt_get_veriff_status()
{
  ...
  $user_information = MT_Api::fetch_user_by_email(email: $user->user_email);
  if ($user_information) {
    wp_send_json_success([
      'status' => $user_information['kycStatus'],
      'verified' => $user_information['kycStatus'] === 'Verified',
    ]);
...
function mt_veriff_callback_handler(WP_REST_Request $request)
{
  ...
  if (isset($data['code']) && $data['code']) {
    return new WP_REST_Response(['success' => true, 'message' => 'in progress'], 200);
  }

  $status = sanitize_text_field($data['verification']['status']);
  $verified = $status === 'approved' ? 'Verified' : 'Not Verified';

  $user_id = intval($data['verification']['vendorData'] ?? 0);
  $email = get_the_author_meta('user_email', $user_id);

  if (!$email) {
    return new WP_REST_Response(['success' => false, 'message' => 'email not found'], 200);
  }

  try {
    $user_information = MT_Api::fetch_user_by_email(email: $email);
    if ($user_information) {
      MT_Api::update_user_by_email(array_merge(["email" => $email], [
          "kycStatus" => $verified,
    ...
    error_log('Error updating user information from mt_veriff_callback_handler: ' . $e->getMessage());
  }
