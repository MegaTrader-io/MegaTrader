...
export function TotalProfit({overallPerformance}: { overallPerformance: OverallPerformance }) {
    const percentage = Math.abs(overallPerformance.totalProfit.percentage);
    const legend = overallPerformance.totalProfit.percentage > 0 ? '+' : '-';
    ...
    return <div className="flex items-center gap-2">
        ...
        <div className="flex">
            {overallPerformance.totalProfit.percentage > 0
                ? <ArrowUp circle={false} className="text-teal-400"/>
                : <ArrowDown circle={false} className="text-rose-500"/>
            }
            <Badge className="!py-0 !text-xs" shape={'pill'} size={'sm'}
                   variant={overallPerformance.totalProfit.percentage > 0 ? 'secondary' : 'error'}>
                {legend} {percentage.toFixed(2)}%
            </Badge>
        </div>
    </div>
}