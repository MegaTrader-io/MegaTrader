...
import PayoutsTable from "@/app/(backoffice)/payouts/_components/PayoutsTable";

export type PayoutStatus = 'all_payouts' | 'approved' | 'pending' | 'rejected'
...
const Options: { id: PayoutStatus, label: string }[] = [
    {id: 'all_payouts', label: 'All Payouts'},
    {id: 'approved', label: 'Approved'},
    {id: 'pending', label: 'Pending'},
    {id: 'rejected', label: 'Rejected'}
]

function SelectionTab({onClick, selection}: { onClick: (option: IPayoutOption) => void, selection: PayoutStatus }) {
    return <>
        {Options.map(option => (
...
function PayoutsManager() {
    const [selection, setSelection] = useState<PayoutStatus>(Options[0].id);

    function clickOption(option: IPayoutOption) {
        setSelection(option.id)
    }

    function changeOption(ev: React.ChangeEvent<HTMLSelectElement>) {
        setSelection(ev.target.value as PayoutStatus)
    }
    ...
    return (
        <div className="w-full space-y-2 lg:space-y-4">
            <Card id="traffic-conversion-table" className="space-y-4">
                ...
                <PayoutsTable payoutStatus={selection}/>
            </Card>
        </div>
    )
}
