...
export function CopyButton({value, className, color = '#A8A29E', disabled = false}: {
    ...
    const id = `mask0_4397_${(new Date()).getTime()}`
    return (
        <>
            <button ref={buttonRef} className={clsx('relative disabled:text-stone-600 disabled:cursor-not-allowed', [className, `text-[${color}]`])} disabled={disabled}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <mask id={id} style={{maskType: 'alpha'}} maskUnits="userSpaceOnUse" x="0" y="0"
                          width="24" height="24">
                ...
                {copySuccess && (
                    <div className="absolute top-[-20px] right-[-9px] text-xs text-gray-400 flex items-center">
                        copied
                    </div>
                )}
            </button>
