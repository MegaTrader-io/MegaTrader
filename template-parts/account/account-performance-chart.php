...
<script>
  const RAW_SERIES  = <?php echo wp_json_encode($js_series); ?>; 
  ...
  function getChartConfig(len) {
    return {
      ...
      colors: ["#ffb34a", "#14B8A6", "#F43F5E"], 
      stroke: { lineCap: "round", curve: "smooth", width: [2,2,2] },
      markers: { size: [0,5,5], colors: ["#F43F5E","#14B8A6"], strokeColors: 'transparent', strokeWidth: 0 },
      legend: { show: false },
      xaxis: {
        categories: [], // 1..N
      ...
      tooltip: {
        ...
        x: { formatter: (val, {dataPointIndex}) => (window.__DATE_LABELS__?.[dataPointIndex] ?? val) },
