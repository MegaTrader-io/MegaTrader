...
<div class="account-selection-wrapper d-flex gap-2 align-items-center gap-3 p-3 rounded-2xl bg-1e1e1e justify-content-between">
  <button type="button" class="mega-btn-md mega-btn-secondary-md flex-shrink-0 flex-grow-1"
  ...
  <?php /* Reset buttons ocultos por ahora
  ...
  <div class="modal-dialog modal-dialog-centered modal-fullscreen-md-down">
    <div class="modal-content gap-4">
      ...
      <div class="modal-body">
        <!-- Select de filtro (solo 3 estados) -->
        <div class="mb-3">
          <label for="mt-acc-filter" class="visually-hidden">Status</label>
          <select id="mt-acc-filter" class="form-select w-100">
        ...
        <div class="subscription-scroll-area px-lg-3 p-0">
          <div class="subscription-grid" id="mt-accounts-grid">
            <?php foreach ($accounts as $a): ?>
              ...
              $isCur      = ($aid === $currentId);
              $card_class = 'subscription-card position-relative d-flex flex-column gap-2';
...
$payload = [
  ...
  'selectors'      => [
    ...
    'performance' => '.mt-account-performance', // contenedor de performance
  ],
  'ajax' => [
...
(function(){
  ...
  function applyFilter(val){
    ...
    grid.querySelectorAll(CFG.selectors.card).forEach(function(card){
      ...
      // mostrar solo si coincide con ACTIVE/BREACHED/PASSED; el resto se oculta
      var show = (st === want);
      card.style.display = show ? '' : 'none';
    });

    // Si el seleccionado quedó oculto, limpiar selección y botón
    var active = grid.querySelector(CFG.selectors.card+'.'+CFG.selectionClass);
  ...
  if (!grid || !btn) {
    // Permitimos que el filtro funcione aun si no existe el botón/DOM completo
    if (filterSel) {
  ...
  grid.addEventListener('click', function(e){
    ...
    if (!card) return;
    if (card.style.display === 'none') return; // no seleccionar ocultos
    selectCard(card);
  });
  ...
  if (selectedId){
    ...
    if (cur) selectCard(cur); else setBtnEnabled(false);
  } else {
    setBtnEnabled(false);
  }

  // Filtro: por defecto ACTIVE y listener
  if (filterSel){
  ...
  // Actualización por AJAX (performance + cabecera)
