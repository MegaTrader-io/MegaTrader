...
elseif (isset($args['accountId']))
  $acc_id = (string) $args['accountId'];
elseif (isset($current_account_id))
  $acc_id = (string) $current_account_id;
if ($acc_id === '')
  $acc_id = (string) get_user_meta($user_id, 'mt_current_account_id', true);

$per_page = isset($args['data']['per_page']) ? (int) $args['data']['per_page'] : 10;
?>

<div id="<?php echo esc_attr($root_id); ?>" class="mt-card" data-account-id="<?php echo esc_attr($acc_id); ?>"
  data-per-page="<?php echo (int) $per_page; ?>">

  <!-- Header -->
  <div class="tr-headbar">
    <div class="d-flex align-items-center gap-2">
      <div class="tr-title"><?php echo esc_html(Label::META_ACCOUNT_OVERVIEW_TOOLTIP['tradesTable']); ?></div>
      <span class="mt-tooltip">
        ...
        <span class="mt-tooltip__panel" role="tooltip">
          <div class="mt-tooltip__title"><?php echo esc_html(Label::META_ACCOUNT_OVERVIEW_TOOLTIP['tradesTable']); ?>
          </div>
          <div class="mt-tooltip__body"><?php echo esc_html(Label::META_ACCOUNT_OVERVIEW_TOOLTIP['tradesTableBody']); ?>
          </div>
        </span>
      </span>
    </div>
    <div class="tr-seg w-308px" role="group" aria-label="Trades filter">
      <button type="button" class="tr-seg__btn mt-btn mt-btn--xs w-100" data-type="CLOSED"
        aria-pressed="true">Close</button>
      <button type="button" class="tr-seg__btn mt-btn mt-btn--xs w-100" data-type="OPEN"
