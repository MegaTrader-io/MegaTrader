...
/** ==== BEGIN: Hydrate data for <div class="mt-card"> (copiado de step_2) ==== */
...
if (!function_exists('mt_get_attr_label')) {
    ...
    {
        if (!$product || !$taxonomy) return '';
        $val = $product->get_attribute($taxonomy);
        if (!empty($val)) return $val; // ej. "$150,000"
...
if (!function_exists('mt_build_rich_debug_payload')) {
    function mt_build_rich_debug_payload($product = null)
    {
        ...
        if (!($product instanceof WC_Product)) return $payload;
        ...
        foreach ($taxos as $tx) {
            ...
            $term = null;
            if ($slug)      $term = get_term_by('slug', $slug, $tx);
            elseif ($label) $term = get_term_by('name', $label, $tx);
            ...
            if ($term && !is_wp_error($term)) {
                ...
                if (!is_array($custom_repeater)) $custom_repeater = $custom_repeater ? (array)$custom_repeater : [];
        ...
        $custom_meta_keys = [
            'profit_target','max_contracts','daily_loss_limit','daily_loss_limit_soft_breach',
            'trailing_max_drawdown','drawdown_mode','min_trading_days','min_trading_days_to_payout',
            'reset_fee','activation_fee','consistency','max_accounts','objectives_rules'
        ];
...
/* --------- Stub de Label si no existe (evita errores fatales) --------- */
if (!class_exists('Label')) {
    class Label {
        const PRICE    = ['price_sufix' => 'PRICE'];
        const PLATFORM = ['title' => 'Platform'];
        // define aquí PRODUCT_META si luego la necesitas en este template
        // const PRODUCT_META = [...];
    }
}
                    ...
                    <div class="two-columns__col">
                        ...
                        <?php if (function_exists('WC') && WC()->cart): ?>
                            ...
                            <?php if ($has_subscription && !empty($addon_options)): ?>
                                <div class="single-checkout-widget checkout-addons mb-32">
                                    <div class="available-info">
                                        <h3 class="main-title">Available Add-ons</h3>
                                        <?php foreach ($addon_options as $option_key => $option_data): ?>
                                            <div class="addons-item <?php echo esc_attr($option_key); ?>">
                                                <div class="title">
                                                    <?php
