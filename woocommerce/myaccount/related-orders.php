...
<div class="account-box mb-lg-down-32 overflow-x-auto d-none">
	<h3 class="box-title"><?php esc_html_e( 'Related orders', 'woocommerce-subscriptions' ); ?></h3>
	...
	<table class="shop_table shop_table_responsive my_account_orders woocommerce-orders-table woocommerce-MyAccount-orders woocommerce-orders-table--orders">
		...
		<thead>
			<tr>
				...
				<th class="woocommerce-orders-table__header woocommerce-orders-table__header-product-name"><span class="nobr"><?php esc_html_e( 'Challenge Name', 'woocommerce-subscriptions' ); ?></span></th>
		...
		<tbody>
			<?php foreach ( $subscription_orders as $subscription_order ) :
				...
				?><tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?>">
					...
					<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-product-name" data-title="<?php esc_attr_e( 'Challenge Name', 'woocommerce-subscriptions' ); ?>">
						<?php
					...
					<td class="order-date woocommerce-orders-table__cell woocommerce-orders-table__cell-order-date" data-title="<?php esc_attr_e( 'Date', 'woocommerce-subscriptions' ); ?>">
						<time datetime="<?php echo esc_attr( $order_date->date( 'Y-m-d' ) ); ?>" title="<?php echo esc_attr( $order_date->getTimestamp() ); ?>"><?php echo esc_html( date_i18n( 'M j, Y', strtotime( $order->get_date_created() ) ) ); ?></time>
					</td>
